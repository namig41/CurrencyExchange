...
class SQLiteCurrenciesRepository(BaseCurrenciesRepository):
    ...
    def check_currency_exists_by_id(self, currency_id: int) -> bool:
        currency: Currency = self.currencies_dao.find_by_id(currency_id)
        return currency is not None

    def get_currencies(self) -> Iterable[Currency]:
        currenices: list[Currency] = [
    ...
    def get_currency_by_id(self, currency_id: int) -> Currency | None:
        currency_data: dict = self.currencies_dao.find_by_id(currency_id)
...
class SQLiteExchangeRatesRepository(BaseExchangeRatesRepository):
    ...
    def get_exchange_rate_by_id(
        ...
        exchange_rate_data: dict = self.exchange_rates_dao.find_by(
            BaseCurrencyId=base_currency.id,
    ...
    def get_exchange_rate_by_codes(
        ...
        exchange_rate_data: dict = self.exchange_rates_dao.find_by_codes(
            base_code,
            target_code,
        )
